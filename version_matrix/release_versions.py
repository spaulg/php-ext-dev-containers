...
def fetch_version_metadata(version_number: str) -> dict:
    ...
    return {
        "release_asset_url": _extract_release_url(version_number, version_metadata),
        "release_date": _extract_release_datetime(version_metadata),
    }


def _extract_release_url(version_number: str, version_metadata: dict) -> str:
    """
    ...
    for file_metadata in version_metadata["source"]:
        if file_metadata["filename"].endswith(".tar.gz"):
            return _expand_release_url(major, file_metadata["filename"], museum)
